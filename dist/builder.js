...
async function runByteHideShield(filePath) {
...
async function runFreeObfuscator(filePath) {
...
async function applyExternalProtections(filePath) {
    try {
...
function isHighValuePreviewAsset(normalizedAssetPath, gameKey) {
    const lower = normalizedAssetPath.toLowerCase();
    if (gameKey !== "olympus")
        return false;
    if (/\.(mp3|ogg|wav|m4a)$/i.test(lower))
        return true;
    return false;
}
function buildPreviewPlaceholderDataUri(assetPath) {
...
function buildTrashWatermarkedImageDataUri() {
    const tinyNoise = "iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAQAAAC1+jfqAAAAIUlEQVR42mNgQAX/Gf4zQAFjYGBg+M8ABYwMDAyMDAwAAG0hBCE4xY0zAAAAAElFTkSuQmCC";
...
async function inlineLocalAssetsInHtml(htmlPath, workDir, options) {
    ...
    for (const ref of uniqueRefs) {
        ...
        if (!normalized || normalized.includes("..") || /^(https?:)?\/\//i.test(normalized) || normalized.startsWith("data:"))
            continue;
...
function buildPreviewGuardPayload(config) {
    const fields = [
        String(config.game ?? ""),
...
async function wrapHtmlWithEncodedBootstrap(htmlPath) {
    const html = await fs.readFile(htmlPath, "utf-8");
    const codecs = ["rot13", "shift"];
    const shiftBy = (Math.floor(Math.random() * 15) + 3);
    const pick = () => codecs[Math.floor(Math.random() * codecs.length)];
    const steps = ["b64", pick(), pick(), pick(), pick()];
    const applyEncode = (input, step) => {
        if (step === "b64")
    ...
    for (const step of steps)
        encoded = applyEncode(encoded, step);
    const stepsJson = JSON.stringify(steps);
    const wrapped = "<!doctype html><html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"></head>" +
...
function getLibraryArtifactPath(order) {
...
async function ensureWorkDependencies(templateConfig, templateDir, workDir) {
    ...
    async function resetWorkNodeModules() {
