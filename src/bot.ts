...
const GAME_CHANNEL_POSTS: Partial<Record<string, string>> = {
...
function getChannelPostForGame(gameKey: string | undefined): string | null {
...
export function registerHandlers(bot: Bot<MyContext>) {
    ...
    bot.callbackQuery(GAMES.RAILROAD.ID, async (ctx) => {
        ...
        const keyboard = new InlineKeyboard()
            .url("👀 Смотреть демо в канале", getChannelPostForGame(GAMES.RAILROAD.GAME_KEY) ?? "https://t.me/rwbrr")
            .row()
    ...
    async function showProductPhotoCard(ctx: MyContext, game: OrderableGame) {
        ...
        const keyboard = new InlineKeyboard();
        if (demoUrl) {
            keyboard.url("👀 Смотреть демо в канале", demoUrl).row();
        }
        keyboard
    ...
    bot.callbackQuery(GAMES.PLINKO.ID, async (ctx) => {
        ...
        await editOrReply(
            ...
            new InlineKeyboard()
                .url("👀 Смотреть демо в канале", demoUrl ?? "https://t.me/rwbrr")
                .row()
                .text("🔙 Назад", CATEGORIES.PLINKO),
        );
    });
    ...
    bot.callbackQuery("gen_preview", async (ctx) => {
        ...
        try {
            ...
            const ctaText = c.clickUrl ?? "не задана";
            ...
            await editOrReply(
                ctx,
                `${demoLine}\n\n🌍 <b>Выбранное GEO:</b> <code>${selectedGeo}</code>\n🔗 <b>CTA-ссылка:</b> <code>${escapeHtml(ctaText)}</code>\n\n💸 <b>${discountCaption}</b>\n${singleLine}\n${subLine}\n\n<i>Проверьте демо и выберите формат покупки:</i>`,
                new InlineKeyboard()
                    .url("👀 Смотреть демо в канале", demoUrl ?? "https://t.me/rwbrr")
                    .row()
            ...
            await editOrReply(ctx, "Ошибка при подготовке демо. Попробуйте снова через минуту.", withBackToMenu);
        } finally {
    ...
    bot.callbackQuery(/^admin_manual_(approve|reject)_/, async (ctx) => {
        ...
        if (action === "approve") {
            await editOrReply(ctx, `⏳ Одобряю заказ ${orderId}. Собираю финальный файл...`, mainMenuNav);
